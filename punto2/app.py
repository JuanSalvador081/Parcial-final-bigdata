...
# import os  # F401: 'os' imported but unused - REMOVED

# --- Configuración S3 ---
S3_BUCKET_NAME = "parcialfinal-juansalvador"
...
def upload_to_s3(file_content, object_name):
    ...
    try:
        s3_client.put_object(
            ...
            Key=object_name,
            Body=file_content,
            ContentType='text/html' if object_name.endswith('.html') else 'text/csv'
        )
            ...
            for card in article_cards:
                ...
                if not title_link_tag and ('card-body' in selector or 'container' in selector or 'item' in selector):
                    ...
                    for link_candidate in candidate_links:
                        ...
                        if link_text and len(link_text) > 10 and not link_text.isdigit() and not \
                                link_candidate.find_parent(class_=lambda c: c and ('footer' in c or 'nav' in c)):
                    ...
                    if not title_link_tag and candidate_links:
                        for link_candidate in candidate_links:
                            if link_candidate.get_text(strip=True) and \
                                    not link_candidate.get_text(strip=True).isdigit():
                                title_link_tag = link_candidate
                                break
                ...
                if title_link_tag:
                    ...
                    if not title_text_candidate and title_link_tag.parent and \
                            title_link_tag.parent.name.startswith(('h1', 'h2', 'h3', 'h4')):
...
def handler(event, context):
    ...
    for record in event['Records']:
        ...
        if not object_key.startswith(f"{S3_FINAL_PREFIX}/") and \
           object_key.endswith('.html') and \
           object_key.startswith(f"{S3_RAW_PREFIX}/"):
            try:
                ...
                try:
                    ...
                    print(f"Advertencia: No se pudo parsear la fecha del nombre del archivo '{filename}' ({e_date})."
                          " Usando fecha actual para el path S3.")
                    now = datetime.now()
                    year = now.strftime("%Y")
            ...
            print(f"Archivo {object_key} no elegible para procesamiento (no es HTML bajo {S3_RAW_PREFIX}/"
                  f" o ya está en {S3_FINAL_PREFIX}/).")
...
if __name__ == '__main__':
    ...
    print("\nEjecución local de la función de procesamiento completada.")